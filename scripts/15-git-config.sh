...
if [[ ! -f "$GIT_KEY" ]]; then
  sudo_admin ssh-keygen -t ed25519 -f "$GIT_KEY" -N "" -C "${GIT_USER_EMAIL}"
else
...
sudo_admin ssh-keyscan -t ed25519 github.com gitlab.com >> "${ADMIN_HOME}/.ssh/known_hosts" 2>/dev/null || true
